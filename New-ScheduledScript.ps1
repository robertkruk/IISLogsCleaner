...
.NOTES
  ...
  GitHub: https://github.com/adbertram/Random-PowerShell-Work/blob/master/Scheduled%20Tasks/New-ScheduledScript.ps1
.EXAMPLE
  PS C:\Scripts> .\New-ScheduledScript.ps1 -ScriptFilePath C:\scripts\IISLogsCleaner\IISLogsCleaner.ps1 -LocalScriptFolderPath 'C:\' -TaskTriggerOptions @{'Daily' = $true; 'At' = '3Am'} -TaskName 'IISLogsCleaner' -TaskRunAsUser 'NT AUTHORITY\SYSTEM' -Computername localhost
		...
		try {
			...
			if ($Computername -ne 'localhost') {
				$ScriptBlock = {
					...
					Register-ScheduledTask -TaskName $using:TaskName -InputObject $Task -User $using:TaskRunAsUser  #-Password $using:TaskRunAsPassword# hased out as using localsystem account without password
				}
				$Params = @{
				...
				$Trigger = New-ScheduledTaskTrigger @TaskTriggerOptions
				$Task = New-ScheduledTask -Action $Action -Trigger $Trigger -Settings (New-ScheduledTaskSettingsSet);
				Register-ScheduledTask -TaskName $TaskName -InputObject $Task -User $TaskRunAsUser  #-Password $using:TaskRunAsPassword# hased out as using localsystem account without password
			}
		} catch {
